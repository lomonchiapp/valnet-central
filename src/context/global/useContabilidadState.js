...
export const useContabilidadState = create()((set, get) => ({
    cuentas: [],
    asientos: [],
    diarioGeneral: [],
    libroDiario: [],
    ...
    subscribeToCuentas: () => {
        const unsubscribe = onSnapshot(collection(database, 'cuentas'), (snapshot) => {
            const cuentasData = snapshot.docs.map((doc) => doc.data());
    ...
    subscribeToAsientos: () => {
        const unsubscribe = onSnapshot(collection(database, 'asientos'), (snapshot) => {
            set({
                asientos: snapshot.docs.map((doc) => doc.data()),
            });
        });
        return unsubscribe;
    },
    subscribeToDiarioGeneral: () => {
        const unsubscribe = onSnapshot(collection(database, 'diarioGeneral'), (snapshot) => {
            set({
                diarioGeneral: snapshot.docs.map((doc) => doc.data()),
            });
        });
        return unsubscribe;
    },
    subscribeToLibroDiario: () => {
        const unsubscribe = onSnapshot(collection(database, 'libroDiario'), (snapshot) => {
            set({
                libroDiario: snapshot.docs.map((doc) => doc.data()),
            });
        });
        return unsubscribe;
    },
    subscribeToReportes: () => {
        const unsubscribe = onSnapshot(collection(database, 'reportes'), (snapshot) => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            set({ reportes: snapshot.docs.map((doc) => doc.data()) });
        });
        return unsubscribe;
    },
    ...
    subscribeToIngresos: () => {
        const unsubscribe = onSnapshot(collection(database, 'ingresos'), (snapshot) => {
    ...
    subscribeToMovimientosCuenta: () => {
        const unsubscribe = onSnapshot(collection(database, 'movimientosCuenta'), (snapshot) => {
    ...
    deletePagoUnico: async (id) => {
        try {
            await runTransaction(database, async (transaction) => {
                ...
                const movimientoAsociado = movimientosCuenta.find((m) => m.origen === OrigenMovimiento.PAGO_UNICO && m.idOrigen === id);
    ...
    deleteIngreso: async (id) => {
        try {
            await runTransaction(database, async (transaction) => {
                ...
                const movimientoAsociado = movimientosCuenta.find((m) => m.origen === OrigenMovimiento.INGRESO && m.idOrigen === id);
                if (movimientoAsociado) {
                    // Buscar la cuenta afectada
                    const cuentaAfectada = cuentas.find((c) => c.id === movimientoAsociado.idcuenta);
                    if (cuentaAfectada) {
