...
export const useCrearNotificacion = () => {
  const crearNotificacion = async (notificacion: NotificacionInput) => {
    try {
...
export const useObtenerNotificaciones = () => {
  const obtenerNotificaciones = async (estado?: EstadoNotificacion) => {
    try {
      let q = query(
        collection(database, 'notificaciones'),
...
export const useArchivarNotificacion = () => {
  const archivarNotificacion = async (id: string) => {
    try {
      const notificacionRef = doc(database, 'notificaciones', id)
      await updateDoc(notificacionRef, {
