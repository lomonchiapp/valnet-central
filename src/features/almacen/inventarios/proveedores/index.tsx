          ...
          {proveedores.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {proveedores.map((proveedor) => (
                <Card key={proveedor.id} className="shadow-md border border-muted-foreground/10">
                  ...
                  <CardContent className="space-y-1 text-sm">
                    <div><span className="font-medium">Teléfono:</span> {proveedor.telefono}</div>
                    <div><span className="font-medium">Email:</span> {proveedor.email}</div>
                    <div><span className="font-medium">Dirección:</span> {proveedor.direccion}</div>
                  </CardContent>
                </Card>
              ))}
            </div>
