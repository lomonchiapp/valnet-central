...
export function ArticulosTable({ articulos }: ArticulosTableProps) {
  ...
  const [editingEquipo, setEditingEquipo] = useState<ExtendedArticulo | null>(null);
  const [showEditForm, setShowEditForm] = useState(false);
  const [selectedArticulo, setSelectedArticulo] = useState<ExtendedArticulo | null>(null);
  ...
  const groupedEquipos: GroupedEquipo[] = equipos.reduce((groups: GroupedEquipo[], equipo) => {
  ...
  const filteredGroupedEquipos = groupedEquipos.filter((group) =>
  ...
  const renderEquipoCard = (articulo: ExtendedArticulo) => {
    ...
    return (
      <Card key={articulo.id} className="overflow-hidden hover:shadow-md transition-all">
        ...
        <CardHeader className="p-4 pb-2">
          <div className="flex justify-between items-start">
            <CardTitle className="text-base line-clamp-1">{articulo.nombre}</CardTitle>
            <DropdownMenu>
        ...
        <CardFooter className="p-4 pt-0 flex justify-between text-sm">
          <span className="text-muted-foreground">{formatCurrency(articulo.costo)}</span>
          {articulo.garantia && articulo.garantia > 0 && (
            <span className="text-xs text-green-600">Garantía: {articulo.garantia} meses</span>
          )}
        </CardFooter>
      </Card>
    );
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="cursor-pointer" onClick={() => handleSort("marca")}>
              ...
              {viewMode !== "materiales" && (
                <TableHead className="cursor-pointer" onClick={() => handleSort("serial")}>
                      ...
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button variant="ghost" size="icon" onClick={() => handleOpenSalidaForm(articulo)}>
  ...
  const renderGroupedEquiposTable = () => {
    return (
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead className="w-[200px] cursor-pointer" onClick={() => handleSort("nombre")}>
              Nombre {renderSortIcon("nombre")}
            </TableHead>
            <TableHead className="cursor-pointer" onClick={() => handleSort("marca")}>
              Marca {renderSortIcon("marca")}
        ...
        <TableBody>
          {filteredGroupedEquipos.map((group) => (
            <TableRow key={group.key}>
              ...
              <TableCell className="text-center">{group.equipos.length}</TableCell>
              ...
              <TableCell className="text-right">{formatCurrency(group.totalCosto)}</TableCell>
              <TableCell className="text-right">
                <div className="flex justify-end space-x-1">
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button variant="ghost" size="icon" onClick={() => handleViewSerials(group)}>
                          <Eye className="h-4 w-4" />
                        </Button>
                      </TooltipTrigger>
                      <TooltipContent>
        ...
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-xl">
              {selectedGroup.nombre} - {selectedGroup.marcaNombre} {selectedGroup.modelo}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4">
            {selectedGroup.equipos.map((equipo) => (
              <Card key={equipo.id} className="p-4 hover:shadow-md transition-all">
                <div className="flex justify-between items-start">
