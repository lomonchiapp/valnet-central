...
export function useActualizarArticulo() {
  ...
  const actualizarArticulo = async (data: ActualizarArticuloParams): Promise<boolean> => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      setError(err instanceof Error ? err : new Error('Error desconocido al actualizar el artículo'));
      return false;
    } finally {
      setIsLoading(false);
    }
