...
import { addDoc, collection, doc, serverTimestamp, updateDoc, query, where, getDocs, onSnapshot, } from 'firebase/firestore';
export function useUbicaciones() {
    ...
    const cargarUbicaciones = async () => {
        ...
        try {
            ...
            const ubicacionesList = snapshot.docs.map((doc) => doc.data());
     ...
     */
    const crearUbicacion = async (data) => {
        setIsLoading(true);
        setError(null);
        try {
    ...
    const actualizarUbicacion = async (data) => {
        ...
        try {
            ...
            const ubicacionesRef = collection(database, 'ubicaciones');
            const q = query(ubicacionesRef, where('nombre', '==', data.nombre.trim()), where('id', '!=', data.id));
            const querySnapshot = await getDocs(q);
            if (!querySnapshot.empty) {
