...
import { PlusCircle, Archive, Warehouse, MapPin, Tag, Truck } from "lucide-react"; // Iconos
...
export default function InventoriosLayout() {
  ...
  const getIconForType = (tipo: TipoInventario | undefined) => { 
    switch (tipo) {
      case TipoInventario.LOCAL: return <Warehouse className="w-5 h-5 mr-2" />; 
      case TipoInventario.BRIGADA: return <Archive className="w-5 h-5 mr-2" />;
      default: return <Archive className="w-5 h-5 mr-2" />;
    }
  };
  ...
  const getTitle = () => {
    ...
    return (
      <>
        ...
        <p className="text-muted-foreground">Administra inventarios, ubicaciones, marcas y proveedores.</p>
      </>
    );
  };
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 space-y-6">
      <Tabs value={activeTab} defaultValue={activeTab} className="w-full" onValueChange={(value) => {
        switch(value) {
          case "inventarios":
