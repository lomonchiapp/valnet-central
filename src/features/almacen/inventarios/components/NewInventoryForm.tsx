...
export const NewInventoryForm: React.FC<NewInventoryFormProps> = ({ onClose, onSuccess }) => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <DialogHeader>
        ...
        <DialogDescription>
          Completa los detalles para registrar un nuevo inventario. Si es el primero,
          considera nombrarlo "Inventario Principal" y tipo "Local".
        </DialogDescription>
      </DialogHeader>
      ...
      <div className="grid gap-4 py-2">
        ...
        <div className="grid grid-cols-4 items-center gap-4">
          ...
          <Controller
            ...
            render={({ field }) => (
              <Input id="nombre" {...field} placeholder="Ej: Inventario Principal" className="col-span-3" />
            )}
          />
        </div>
        {errors.nombre && <p className="text-red-500 text-xs col-start-2 col-span-3">{errors.nombre.message}</p>}
          ...
          <Controller
            ...
            render={({ field }) => (
              <Select onValueChange={field.onChange} defaultValue={field.value} value={field.value}>
                ...
                <SelectContent>
                  <SelectItem value={TipoInventario.LOCAL}>Local (Ej: Principal, Cocina, Almacén)</SelectItem>
                  <SelectItem value={TipoInventario.BRIGADA}>Brigada</SelectItem>
                </SelectContent>
              </Select>
            )}
          />
        </div>
        {errors.tipo && <p className="text-red-500 text-xs col-start-2 col-span-3">{errors.tipo.message}</p>}
        ...
        <div className="grid grid-cols-4 items-start gap-4"> {/* items-start para Textarea */}
          ...
          <Controller
            name="descripcion"
            control={control}
            rules={{ maxLength: { value: 500, message: 'La descripción no puede exceder los 500 caracteres' } }} // Ejemplo de otra regla
            render={({ field }) => (
              <Textarea
                id="descripcion"
                {...field}
        ...
        {errors.descripcion && <p className="text-red-500 text-xs col-start-2 col-span-3">{errors.descripcion.message}</p>}
      </div>
