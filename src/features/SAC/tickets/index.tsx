...
export default function Tickets() {
  ...
  const filteredTickets = tickets.filter(ticket => {
    return (
      ...
      (!filterRemitente || ticket.solicitante.toLowerCase().includes(filterRemitente.toLowerCase())) &&
      (!filterAsunto || ticket.asunto.toLowerCase().includes(filterAsunto.toLowerCase())) &&
      (!filterTecnico || (ticket.brigada?.nombre || 'No asignado') === filterTecnico)
    );
  });
    ...
    <div className="container mx-auto py-8 px-4 md:px-8 lg:px-16">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-8">
        <div className="flex items-center gap-3">
          <RefreshCw className={loading ? "animate-spin h-6 w-6 text-primary" : "h-6 w-6 text-primary"} />
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight">Tickets</h1>
        </div>
        <Button variant="default" className="h-10 px-6">+ Nuevo</Button>
      </div>
      <div className="bg-white rounded-xl shadow-md p-4 md:p-8">
        ...
        <div className="flex flex-wrap gap-4 mb-4">
          ...
          >
            ...
            {brigadas.map(brig => (
              <option key={brig.id} value={brig.nombre}>{brig.nombre}</option>
            ))}
          </select>
        </div>
        <Table>
          ...
          <TableBody>
            {paginatedTickets.map((ticket) => (
              <TableRow key={ticket.id}>
                <TableCell>
                  <Button variant="outline" size="icon" className="mr-2 h-6 w-6 p-0">
                    +
                  </Button>
