  ...
  return (
    <Card className='max-w-xl mx-auto mt-8 shadow-lg'>
      ...
      <CardContent className='space-y-2'>
        ...
        <div>
          <span className='font-medium'>Solicitante:</span> {ticket.solicitante}
        </div>
        <div>
          <span className='font-medium'>Fecha:</span>{' '}
          {ticket.fecha ? new Date(ticket.fecha).toLocaleDateString() : ''}
        </div>
        <div>
          <span className='font-medium'>Motivo de cierre:</span>{' '}
          {ticket.motivo_cierre || 'N/A'}
        </div>
        <div>
          <span className='font-medium'>Turno:</span> {ticket.turno}
        </div>
        <div>
          <span className='font-medium'>Departamento:</span> {ticket.dp}
        </div>
      </CardContent>
    </Card>
  )
