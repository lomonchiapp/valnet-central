...
export type IngresoInput = Omit<Ingreso, 'id' | 'createdAt' | 'updatedAt'>

interface UseIngresosReturn {
  isLoading: boolean
  createIngreso: (ingresoData: IngresoInput) => Promise<Ingreso | null>
  updateIngreso: (id: string, ingresoData: Partial<IngresoInput>) => Promise<Ingreso | null>
  deleteIngreso: (id: string) => Promise<boolean>
  validateIngresoData: (ingresoData: IngresoInput) => boolean
}
...
export function useIngresos(): UseIngresosReturn {
  ...
  const validateIngresoData = useCallback((ingresoData: IngresoInput): boolean => {
  ...
  const createIngreso = useCallback(async (ingresoData: IngresoInput): Promise<Ingreso | null> => {
    if (!validateIngresoData(ingresoData)) return null
  ...
  const updateIngreso = useCallback(async (id: string, ingresoData: Partial<IngresoInput>): Promise<Ingreso | null> => {
    setIsLoading(true)
    try {
      const updateData: Partial<Ingreso> = {}
  ...
  const deleteIngreso = useCallback(async (id: string): Promise<boolean> => {
    setIsLoading(true)
    try {
