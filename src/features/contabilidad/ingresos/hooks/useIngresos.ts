...
interface UseIngresosReturn {
  ...
  updateIngreso: (
    id: string,
    ingresoData: Partial<IngresoInput>
...
export function useIngresos(): UseIngresosReturn {
  ...
  const {
    ...
    updateIngreso: updateIngresoContext,
    deleteIngreso: deleteIngresoContext,
  } = useContabilidadState()

  const validateIngresoData = useCallback(
    (ingresoData: IngresoInput): boolean => {
  ...
  const createIngreso = useCallback(
    async (ingresoData: IngresoInput): Promise<Ingreso | null> => {
