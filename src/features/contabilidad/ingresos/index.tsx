...
export default function Ingresos() {
  ...
  const filtered = ingresos.filter(
    (ingreso) =>
      ...
      getCuentaNombre(ingreso.idcuenta).toLowerCase().includes(search.toLowerCase()) ||
      tipoIngresoLabels[ingreso.tipo].toLowerCase().includes(search.toLowerCase())
  )
  ...
  const getEstadisticas = () => {
    ...
    const tipoMasComun = Object.entries(ingresosPorTipo).reduce((a, b) => 
      ingresosPorTipo[a[0] as TipoIngreso] > ingresosPorTipo[b[0] as TipoIngreso] ? a : b
    )?.[0] as TipoIngreso
  ...
  return (
    ...
    >
      ...
      <div className='flex gap-4 items-center justify-between mb-6'>
        ...
        <Button onClick={handleAddIngreso} className="flex items-center gap-2 bg-green-600 hover:bg-green-700">
                ...
                <TableRow key={ingreso.id} className="hover:bg-green-50">
                  ...
                  <TableCell className="max-w-xs">
                    <div className="truncate" title={ingreso.descripcion}>
                      ...
                      {ingreso.notas && (
                        <div className="text-xs text-gray-500 mt-1">{ingreso.notas}</div>
                      )}
                    </div>
                  </TableCell>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              ...
                              de <strong>"{ingreso.descripcion}"</strong> por 
                              <strong> ${ingreso.monto.toLocaleString()}</strong>.
                              <br /><br />
          ...
          <div className="bg-green-50 px-4 py-3 border-t">
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="font-semibold">
                Total mostrado: <span className="text-green-600">${calcularTotal().toLocaleString()}</span>
              </span>
            </div>
          </div>
        )}
