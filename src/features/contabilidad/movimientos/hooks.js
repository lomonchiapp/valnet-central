...
export const useCrearMovimientoCuenta = () => {
    const crearMovimientoCuenta = async (movimiento) => {
        try {
            ...
            const cleanMovimientoData = Object.fromEntries(Object.entries(movimientoData).filter(([, value]) => value !== undefined));
...
export const useObtenerMovimientosCuenta = () => {
    const obtenerMovimientosCuenta = async (idcuenta) => {
        try {
            const q = query(collection(database, 'movimientosCuenta'), where('idcuenta', '==', idcuenta), orderBy('fecha', 'desc'));
...
export const useObtenerMovimientosPorOrigen = () => {
    const obtenerMovimientosPorOrigen = async (origen, idOrigen) => {
        try {
            const q = query(collection(database, 'movimientosCuenta'), where('origen', '==', origen), where('idOrigen', '==', idOrigen));
            const querySnapshot = await getDocs(q);
            return querySnapshot.docs.map((doc) => doc.data());
        }
        catch (error) {
