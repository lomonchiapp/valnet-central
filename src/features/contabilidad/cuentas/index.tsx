...
export default function Cuentas() {
  const [cuentas, setCuentas] = useState<CuentaForm[]>([
    { id: '1', nombre: 'Activos', tipo: 'ACTIVO', descripcion: 'Activos de la empresa', balance: 0 },
    { id: '2', nombre: 'Caja', tipo: 'ACTIVO', descripcion: 'Efectivo en caja', balance: 1000, parentId: '1' },
    { id: '3', nombre: 'Banco', tipo: 'ACTIVO', descripcion: 'Cuenta bancaria', balance: 5000, parentId: '1' },
    { id: '4', nombre: 'Pasivos', tipo: 'PASIVO', descripcion: 'Pasivos de la empresa', balance: 0 },
    { id: '5', nombre: 'Proveedores', tipo: 'PASIVO', descripcion: 'Deudas con proveedores', balance: 2000, parentId: '4' },
  ])
  ...
  const handleAddOrEdit = () => {
    ...
    if (editId) {
      setCuentas(cuentas.map(c => c.id === editId ? { id: editId, nombre, tipo, descripcion, balance: Number(balance), parentId: parentId || undefined } : c))
      setEditId(null)
      toast.success('Cuenta actualizada')
    } else {
      setCuentas([...cuentas, { id: Date.now().toString(), nombre, tipo, descripcion, balance: Number(balance), parentId: parentId || undefined }])
  ...
  return (
    <div className='space-y-6'>
      <div>
        ...
        <p className='text-muted-foreground mb-4'>Configura y personaliza las cuentas contables que hacen parte de tu catálogo.</p>
        <div className='grid grid-cols-1 md:grid-cols-6 gap-4 mt-4'>
          ...
          <select value={parentId} onChange={e => setParentId(e.target.value)} className='border rounded px-2 py-1'>
            ...
            {principales.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}
          </select>
                  ...
                  <TableRow key={cuenta.id} className='bg-gray-50'>
                    <TableCell>
                      {getSubcuentas(cuenta.id).length > 0 && (
                        <Button size='sm' variant='ghost' onClick={() => toggleExpand(cuenta.id)}>
                          {expanded[cuenta.id] ? '−' : '+'}
                        </Button>
                      )}
                      <span className='font-semibold ml-2'>{cuenta.nombre}</span>
                    </TableCell>
                    <TableCell>{cuenta.tipo}</TableCell>
                    <TableCell>{cuenta.descripcion}</TableCell>
                    <TableCell>${cuenta.balance.toLocaleString()}</TableCell>
                    <TableCell>
                      <Button size='sm' variant='outline' onClick={() => handleEdit(cuenta)}>Editar</Button>
                      <Button size='sm' variant='destructive' onClick={() => handleDelete(cuenta.id)} className='ml-2'>Eliminar</Button>
                    </TableCell>
                  </TableRow>
                  {expanded[cuenta.id] && getSubcuentas(cuenta.id).map(sub => (
                    <TableRow key={sub.id} className='bg-white'>
                      ...
                      <TableCell>
                        <Button size='sm' variant='outline' onClick={() => handleEdit(sub)}>Editar</Button>
                        <Button size='sm' variant='destructive' onClick={() => handleDelete(sub.id)} className='ml-2'>Eliminar</Button>
                      </TableCell>
                    </TableRow>
                  ))}
                </>
