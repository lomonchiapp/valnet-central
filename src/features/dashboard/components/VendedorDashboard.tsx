...
function getNivelInfo(contratosMes: number) {
  return (
    niveles.find((n) => contratosMes >= n.min && contratosMes <= n.max) ||
    niveles[0]
  )
}
...
export function VendedorDashboard({
  ...
  const progreso = siguienteNivel
    ? Math.min(
        100,
        (((usuario.contratosMes || 0) - nivelInfo.min + 1) /
              ...
              <div className='flex items-center gap-3'>
                ...
                <div>
                  ...
                  <CardDescription className='text-base font-semibold'>
                    Comisión: {nivelInfo.comision}%
                  </CardDescription>
                </div>
              </div>
              {siguienteNivel && (
                <div className='mt-2 text-sm text-gray-700'>
                  Próximo nivel: <b>{siguienteNivel.nivel}</b> (
                  {siguienteNivel.min} contratos)
                </div>
              )}
            </CardHeader>
            ...
            <CardContent>
              {usuario.bonoExtra ? (
                <span className='text-green-600 font-bold'>
                  ¡Has recibido el bono extra este mes!
                </span>
              ) : (
                <span className='text-muted-foreground'>
                  Aún no has alcanzado el bono extra.
                </span>
              )}
            </CardContent>
          </Card>
              ...
              {preRegistros && preRegistros.length > 0 ? (
                <ul className='space-y-1'>
                  {preRegistros.slice(0, 5).map((pr, idx) => (
                    <li key={idx} className='text-sm'>
                      {pr.descripcion || pr.id}
                    </li>
                  ))}
                </ul>
              ) : (
                <span className='text-muted-foreground'>
                  No hay pre-registros recientes.
                </span>
              )}
            </CardContent>
          </Card>
