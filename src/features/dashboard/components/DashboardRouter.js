...
export function DashboardRouter() {
    ...
    const fetchVendedorData = useCallback(async () => {
        try {
            ...
            const preRegistrosQuery = query(collection(database, 'pre-registros'), where('vendedorId', '==', user?.id));
            ...
            const contratosQuery = query(collection(database, 'contratos'), where('vendedorId', '==', user?.id));
    ...
    const renderDashboard = () => {
        switch (user.role) {
            ...
            case RoleUsuario.VENDEDOR:
                return (_jsx(VendedorDashboard, { usuario: user, preRegistros: preRegistros, contratos: contratos }));
            ...
            case RoleUsuario.CONTABILIDAD:
                return _jsx(GenericDashboard, { inventarios: [], preRegistros: [] });
            default:
