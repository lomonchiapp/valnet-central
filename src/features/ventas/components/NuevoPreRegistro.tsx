...
const formSchema = z.object({
  ...
  movil: z.string().min(8, { message: 'El móvil debe tener al menos 8 dígitos' }),
  email: z.string().email({ message: 'Correo electrónico inválido' }),
  notas: z.string().optional(),
  fecha_instalacion: z.string().min(1, { message: 'La fecha de instalación es requerida' }),
...
export default function NuevoPreRegistro() {
  ...
  const handleFileChange = (
    ...
    const validFiles = filesArray.filter((file) => {
      ...
      if (!isValidType) {
        toast({ title: 'Error', description: `Tipo de archivo no soportado: ${file.name}`, variant: 'destructive' });
      }
      if (!isValidSize) {
        toast({ title: 'Error', description: `Archivo demasiado grande: ${file.name}`, variant: 'destructive' });
      }
  ...
  return (
    <Card className="w-full max-w-3xl mx-auto">
      <CardHeader>
        ...
        <CardDescription>
          Registra un nuevo cliente potencial. Las fotos de la cédula y contrato son opcionales.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Calle Principal #123, Sector X" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="¿Quién refirió a este cliente?" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Cualquier información adicional relevante..." {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <div className="space-y-4">
              <div>
                ...
                <FormDescription>Sube fotos claras de ambos lados de la cédula si están disponibles</FormDescription>
                    ...
                    {fotoCedula.map((file, index) => (
                      <div key={index} className="relative border rounded p-2 flex items-center">
                        ...
                        <Button
                          ...
                          size="icon"
                          className="h-8 w-8"
                          onClick={() => removeFile(index, fotoCedula, setFotoCedula)}
                        >
                          <X className="h-4 w-4" />
                        </Button>
                      </div>
                    ...
                    <div className="flex items-center justify-center border border-dashed rounded-lg p-4">
                      <label className="cursor-pointer flex flex-col items-center gap-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => handleFileChange(e, setFotoCedula, 2)}
                          className="hidden"
                          multiple
                        />
                      </label>
              ...
              <div>
                ...
                <FormDescription>Sube fotos claras del contrato firmado si está disponible</FormDescription>
                <div className="mt-2 flex flex-col gap-2">
                  <div className="grid grid-cols-2 gap-2">
                    {fotoContrato.map((file, index) => (
                      <div key={index} className="relative border rounded p-2 flex items-center">
                        ...
                        <Button
                          ...
                          size="icon"
                          className="h-8 w-8"
                          onClick={() => removeFile(index, fotoContrato, setFotoContrato)}
                        >
                          <X className="h-4 w-4" />
                        </Button>
                      </div>
                    ...
                    <div className="flex items-center justify-center border border-dashed rounded-lg p-4">
                      <label className="cursor-pointer flex flex-col items-center gap-2">
                        ...
                        <Input
                          type="file"
                          accept={ACCEPTED_IMAGE_TYPES.join(',')}
                          onChange={(e) => handleFileChange(e, setFotoContrato, 10)}
                          className="hidden"
                          multiple
                        />
                      </label>
            ...
            <CardFooter className="flex justify-end px-0 pt-4">
              <Button type="button" variant="outline" className="mr-2" onClick={() => navigate('/ventas/pre-registros')}>
                Cancelar
              </Button>
