...
export default function CombustiblePanel({ brigada }: CombustiblePanelProps) {
  const { controlCombustible, subscribeToControlCombustible } =
  ...
  const [registroAEliminar, setRegistroAEliminar] =
    useState<ControlCombustible | null>(null)
  const [deleting, setDeleting] = useState(false)
  const { user } = useAuthStore()
  const [editDialogOpen, setEditDialogOpen] = useState(false)
  const [registroAEditar, setRegistroAEditar] =
    useState<ControlCombustible | null>(null)
  ...
  const registros = useMemo(() => {
    ...
    if (dateFrom)
      filtered = filtered.filter(
        (r) =>
          isAfter(parseISO(r.fecha), parseISO(dateFrom)) || r.fecha === dateFrom
      )
    if (dateTo)
      filtered = filtered.filter(
        (r) =>
          isBefore(parseISO(r.fecha), parseISO(dateTo)) || r.fecha === dateTo
      )
    return filtered
  ...
  const totalGalones = registros.reduce(
    (acc, r) => acc + (Number(r.galones) || 0),
    0
  )
  const totalKm = registros.reduce(
    (acc, r) => acc + ((Number(r.km_final) || 0) - (Number(r.km_inicial) || 0)),
    0
  )
  const rendimiento =
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  variant='outline'
                  onClick={() =>
                    exportToCSV(registros, `combustible_${brigada.nombre}.csv`)
                  }
                  title='Exportar CSV'
                >
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className='max-w-md'>
          <DialogHeader>
            ...
            <DialogDescription>
              Por favor, indica el motivo de la eliminación. Esta acción quedará
              registrada.
            </DialogDescription>
          </DialogHeader>
          <Input
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!deleteMotivo || deleting}
            >
