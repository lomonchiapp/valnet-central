...
function RegistroCombustibleForm({ brigadaId, onCancel }: { brigadaId: string, onCancel: () => void }) {
...
export default function BrigadaDetail() {
  ...
  if (isNuevo) {
    return <RegistroCombustibleForm brigadaId={brigada.id} onCancel={() => navigate(-1)} />;
  }
    ...
    <Card className="max-w-4xl w-full mx-auto mt-8 shadow-lg p-2 md:p-6 relative">
      ...
      <CardHeader className="flex flex-row items-center gap-4 justify-between">
        <div>
          ...
          <CardDescription className="text-sm text-muted-foreground">ID: {brigada.id}</CardDescription>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setOpenInventario(true)} title="Ver inventario">
