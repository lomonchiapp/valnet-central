...
export default function Gastos() {
  const [gastos, setGastos] = useState<GastoForm[]>([
    { id: '1', proveedor: 'Proveedor A', descripcion: 'Compra de papelería', monto: 200, fecha: '2024-06-01' },
    { id: '2', proveedor: 'Proveedor B', descripcion: 'Pago de internet', monto: 100, fecha: '2024-06-02' },
  ])
  ...
  const handleAddOrEdit = () => {
    ...
    if (editId) {
      setGastos(gastos.map(g => g.id === editId ? { id: editId, proveedor, descripcion, monto: Number(monto), fecha } : g))
      setEditId(null)
      toast.success('Gasto actualizado')
    } else {
      setGastos([...gastos, { id: Date.now().toString(), proveedor, descripcion, monto: Number(monto), fecha }])
      ...
      <div className='flex gap-4 items-center flex-wrap'>
        ...
        <Sheet open={open} onOpenChange={setOpen}>
          <SheetTrigger asChild>
            <Button className='ml-auto' onClick={() => { setEditId(null); setProveedor(''); setDescripcion(''); setMonto(''); setFecha(''); setOpen(true) }}>
          ...
          <SheetContent side='top' className='h-[400px] w-full max-w-2xl mx-auto rounded-b-xl border-t-0 animate-in slide-in-from-top duration-300'>
            <SheetHeader className='mb-6'>
              <SheetTitle className='text-2xl'>{editId ? 'Editar Gasto' : 'Agregar Gasto'}</SheetTitle>
            </SheetHeader>
