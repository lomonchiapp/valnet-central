...
interface UsePagosUnicosReturn {
  ...
  updatePago: (id: string, pagoData: Partial<PagoUnicoInput>) => Promise<PagoUnico | null>
...
export function usePagosUnicos(): UsePagosUnicosReturn {
  ...
  const createPago = useCallback(async (pagoData: PagoUnicoInput): Promise<PagoUnico | null> => {
  ...
  const updatePago = useCallback(async (id: string, pagoData: Partial<PagoUnicoInput>): Promise<PagoUnico | null> => {
    setIsLoading(true)
    try {
