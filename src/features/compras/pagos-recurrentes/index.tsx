...
import { PagoRecurrente, EstadoPagoRecurrente, TipoMonto } from '@/types/interfaces/contabilidad/pagoRecurrente'
...
export default function PagosRecurrentes() {
  ...
  const [variablePaymentDialog, setVariablePaymentDialog] = useState<{ show: boolean; pago: PagoRecurrente | null }>({
    show: false,
    pago: null
  })
  ...
  const pagosProximos = pagosRecurrentes.filter((pago: PagoRecurrente) => {
    ...
    return diasRestantes >= 0 && diasRestantes <= 7 && pago.estado === EstadoPagoRecurrente.ACTIVO
  })
  ...
  const totalMontoProximos = pagosProximos.reduce((sum: number, pago: PagoRecurrente) => sum + pago.monto, 0)
  const totalMontoVencidos = pagosVencidos.reduce((sum: number, pago: PagoRecurrente) => sum + pago.monto, 0)

  const getProveedorNombre = (idproveedor: string) => {
    return proveedores.find((p: { id: string; nombre: string }) => p.id === idproveedor)?.nombre || 'Proveedor desconocido'
  }
  ...
  const getEstadoBadge = (pago: PagoRecurrente) => {
    ...
    if (diasRestantes <= 3) {
      return <Badge className="bg-yellow-500 hover:bg-yellow-600">Urgente ({diasRestantes}d)</Badge>
    }
    
    if (diasRestantes <= 7) {
      return <Badge className="bg-blue-500 hover:bg-blue-600">Próximo ({diasRestantes}d)</Badge>
    }
  ...
  const renderAmountCell = (pago: PagoRecurrente) => {
    ...
    const isVariablePayment = String(pago.tipoMonto) === 'VARIABLE' || pago.tipoMonto === TipoMonto.VARIABLE
      ...
      return (
        <TableCell className="text-right">
          <div className="flex flex-col items-end gap-1">
            <Dialog 
              open={variablePaymentDialog.show && variablePaymentDialog.pago?.id === pago.id}
              onOpenChange={(open) => {
                if (!open) {
                  setVariablePaymentDialog({ show: false, pago: null })
                  setVariableAmount('')
              ...
              <DialogContent>
                ...
                <DialogFooter>
                  ...
                  <Button
                    ...
                    disabled={processingPaymentId === pago.id || !variableAmount}
                  >
                    {processingPaymentId === pago.id ? 'Procesando...' : 'Procesar Pago'}
                  </Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
  ...
  const sortedPagosRecurrentes = [...pagosRecurrentes].sort((a: PagoRecurrente, b: PagoRecurrente) => {
    ...
    <div className='space-y-6 max-w-7xl mx-auto'>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card className="border-red-200 bg-red-50">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-900">{pagosVencidos.length}</div>
        ...
        <Card className="border-yellow-200 bg-yellow-50">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-yellow-700">Próximos (7 días)</CardTitle>
            <Clock className="h-4 w-4 text-yellow-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-yellow-900">{pagosProximos.length}</div>
        ...
        <Card className="border-green-200 bg-green-50">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-900">
              {pagosRecurrentes.filter((p: PagoRecurrente) => p.estado === EstadoPagoRecurrente.ACTIVO).length}
            </div>
        ...
        <Card className="border-blue-200 bg-blue-50">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-900">{pagosRecurrentes.length}</div>
              ...
              <Table>
                ...
                <TableBody>
                  {sortedPagosRecurrentes.map((pago: PagoRecurrente) => (
                    <TableRow key={pago.id} className={`
                    ...
                    `}>
                      <TableCell className="font-medium">
                        <div>
                          <div className="font-semibold">{pago.descripcion}</div>
                          {pago.notas && (
                            <div className="text-sm text-gray-500">{pago.notas}</div>
                          )}
                        </div>
                      </TableCell>
                      <TableCell>{getProveedorNombre(pago.idproveedor)}</TableCell>
                      <TableCell className="text-sm">{getCuentaNombre(pago.idcuenta)}</TableCell>
                      ...
                      <TableCell>
                        <div className="text-sm">
                          {format(new Date(pago.fechaProximoPago), 'dd MMM yyyy', { locale: es })}
                        </div>
                      </TableCell>
                      <TableCell>
                        {getEstadoBadge(pago)}
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  Esta acción eliminará permanentemente el pago recurrente
                                  <strong> "{pago.descripcion}"</strong> de
                                  <strong> {getProveedorNombre(pago.idproveedor)}</strong>.
                                  <br /><br />
                                  Esta acción no se puede deshacer.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
                                <AlertDialogCancel disabled={deletingId === pago.id}>
                                  Cancelar
                                </AlertDialogCancel>
                                <AlertDialogAction
                                ...
                                >
                                  {deletingId === pago.id ? 'Eliminando...' : 'Eliminar'}
                                </AlertDialogAction>
                              </AlertDialogFooter>
                            </AlertDialogContent>
                          </AlertDialog>
