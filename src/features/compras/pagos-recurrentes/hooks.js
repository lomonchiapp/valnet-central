...
export const useCrearPagoRecurrente = () => {
    ...
    const crearPagoRecurrente = async (pago) => {
        try {
            ...
            const cleanPagoData = Object.fromEntries(Object.entries(pagoData).filter(([, value]) => value !== undefined));
...
export const useActualizarPagoRecurrente = () => {
    const actualizarPagoRecurrente = async (id, pago) => {
        try {
...
export const useBorrarPagoRecurrente = () => {
    const borrarPagoRecurrente = async (id) => {
        try {
...
export const useProcesarPagoVariable = () => {
    const { crearMovimientoCuenta } = useCrearMovimientoCuenta();
    const procesarPagoVariable = async (id, montoReal) => {
        try {
...
export const useObtenerPagosVariablesPendientes = () => {
    const obtenerPagosVariablesPendientes = async (idcuenta) => {
        try {
            let q = query(collection(database, 'pagosRecurrentes'), where('tipoMonto', '==', 'VARIABLE'), where('estado', '==', EstadoPagoRecurrente.ACTIVO));
            if (idcuenta) {
                q = query(q, where('idcuenta', '==', idcuenta));
            }
            const querySnapshot = await getDocs(q);
            const pagosVariables = querySnapshot.docs.map((doc) => doc.data());
