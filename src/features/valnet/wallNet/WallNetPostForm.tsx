...
const PRIORITY_OPTIONS: { value: WallNetPriority, label: string, color: string, icon: React.ReactNode }[] = [
  { value: 'normal', label: 'Normal', color: VALNET_BLUE, icon: <Star className="w-4 h-4" /> },
  { value: 'importante', label: 'Importante', color: '#eab308', icon: <AlertCircle className="w-4 h-4" /> },
  { value: 'urgente', label: 'Urgente', color: VALNET_ORANGE, icon: <Flame className="w-4 h-4" /> },
]

// Utilidad para comprimir imagen a JPEG de baja calidad
async function compressImage(file: File, maxWidth = 600, maxHeight = 600, quality = 0.5): Promise<Blob> {
...
const WallNetPostForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent | React.KeyboardEvent) => {
    ...
    await postMessage(
      { content, imageUrl: finalImageUrl, audioUrl: finalAudioUrl, categoryId, priority },
  ...
  const handleSendMouseUp = (e: React.MouseEvent<HTMLButtonElement> | React.TouchEvent<HTMLButtonElement>) => {
    if (longPressTimeout.current) {
      clearTimeout(longPressTimeout.current)
      longPressTimeout.current = null
    }
    if (!showPopover) {
      handleSubmit(e as React.MouseEvent<HTMLButtonElement> | React.TouchEvent<HTMLButtonElement>)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex gap-3 items-start relative">
      ...
      <div className="flex-shrink-0 absolute -left-7 top-3">
        <div className="w-10 h-10 rounded-full" style={{ background: VALNET_BLUE, color: '#fff', border: '2px solid #fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: 20 }}>
          {user ? user.nombres[0] : '?'}
        </div>
      </div>
      {/* Formulario */}
      <div className="flex-1 flex gap-2 items-center bg-white rounded-xl border shadow px-3 py-2">
        <textarea
          ...
          maxLength={300}
          required
          onKeyDown={(e: React.KeyboardEvent<HTMLTextAreaElement>) => { if (e.key === 'Enter' && !e.shiftKey) handleSubmit(e) }}
        />
        ...
        <div className="relative">
          <button
            ...
            style={{ background: currentCategory?.color + '22', color: currentCategory?.color, borderColor: currentCategory?.color || VALNET_BLUE }}
            onClick={() => setShowCategory(v => !v)}
            title={currentCategory?.name}
          >
            <span className="truncate max-w-[80px]">{currentCategory?.name || 'Categoría'}</span>
            ...
            <div className="absolute z-30 bg-white border rounded shadow-md mt-2 right-0 min-w-[140px]">
              {categories.map(cat => (
                <button
                  ...
                  style={{ color: cat.color, borderLeft: `4px solid ${cat.color}` }}
                  onClick={() => { setCategoryId(cat.id); setShowCategory(false) }}
                >
                  <span className="w-3 h-3 rounded-full mr-2" style={{ background: cat.color, display: 'inline-block' }}></span>
                  {cat.name}
                </button>
              ))}
            </div>
        ...
        <div className="relative">
          <button
            ...
            style={{ borderColor: currentPriority?.color, color: currentPriority?.color, background: priority === 'urgente' ? VALNET_ORANGE + '22' : priority === 'normal' ? VALNET_BLUE + '22' : '#fffbe6' }}
        ...
        <div className="relative">
          ...
          {showPopover && (
            <div className="absolute z-40 right-0 mt-2 bg-white border rounded shadow-lg p-4 min-w-[180px] flex flex-col gap-3">
              ...
              >
                <Mic className="w-5 h-5" /> {isRecording ? 'Detener grabación' : 'Nota de voz'}
              </button>
              {recordingUrl && (
