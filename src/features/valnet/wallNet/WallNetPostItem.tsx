...
const PRIORITY_STYLE = {
  normal:
    'border-2 border-[#1976d2] shadow-[0_2px_12px_0_rgba(25,118,210,0.08)] hover:shadow-[0_0_0_4px_rgba(25,118,210,0.15)]',
  importante:
    'border-2 border-yellow-400 bg-gradient-to-br from-yellow-50 to-white shadow-[0_2px_12px_0_rgba(250,204,21,0.10)] hover:shadow-[0_0_0_4px_rgba(250,204,21,0.18)]',
  urgente:
    'border-2 border-[#F37021] bg-gradient-to-br from-orange-50 to-white shadow-[0_2px_12px_0_rgba(243,112,33,0.12)] hover:shadow-[0_0_0_4px_rgba(243,112,33,0.20)]',
}
      ...
      >
          ...
          <div className='flex-1 min-w-0 relative'>
            <div className='flex items-center gap-2 mb-1'>
              ...
              <span className='ml-auto'>
                <span
                  className='text-xs px-2 py-0.5 absolute right-0 top-0
            ...
            {editing ? (
              <div className='flex gap-2 items-center mt-2'>
                ...
                />
                <button
                  className='p-2 rounded-full bg-[#0053A0] text-white hover:bg-[#0053A0]/90 transition'
                  title='Guardar'
                  onClick={handleEdit}
                >
            ...
            {post.imageUrl && (
              <img
                src={post.imageUrl}
                alt='post'
                className='max-w-full max-h-60 rounded-lg border mb-2'
              />
            )}
            <div className='flex gap-4 text-sm text-muted-foreground mt-2 items-center relative group'>
              <div className='relative'>
                ...
                {showLikes && post.likes.length > 0 && (
                  <div className='absolute left-0 top-8 z-30 bg-white border rounded shadow-lg min-w-[180px] p-2 animate-fade-in'>
                    <div className='flex items-center gap-2 mb-2 text-[#0053A0] font-semibold'>
                      <Users className='w-4 h-4' /> Likes
                    </div>
                    <ul className='max-h-40 overflow-y-auto text-xs'>
                      {post.likes.map((uid) => (
                        <li
                          key={uid}
                          className='py-1 px-2 hover:bg-blue-50 rounded'
                        >
                          {getUserName(uid)}
                        </li>
                      ))}
              ...
              {isOwner && !editing && (
                <span className='absolute right-0 bottom-0 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10'>
                  <button
                    className='p-1 rounded hover:bg-[#0053A0]/10'
                    title='Editar'
                    onClick={() => {
                      setEditing(true)
                  ...
                  >
                    <Edit2 className='w-4 h-4 text-[#0053A0]' />
                  </button>
                  <button
                    className='p-1 rounded hover:bg-[#F37021]/10'
                    title='Eliminar'
                    onClick={() => setShowDelete(true)}
                  >
                    <Trash2 className='w-4 h-4 text-[#F37021]' />
                  </button>
                </span>
              )}
            </div>
          ...
          <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/30'>
            <div className='bg-white rounded-lg shadow-lg p-6 flex flex-col items-center'>
              ...
              <div className='flex gap-4'>
                <button
                  className='bg-[#F37021] text-white px-4 py-2 rounded hover:bg-[#F37021]/90'
        ...
        {showComments && (
          <div className='mt-3 border-t pt-3'>
            <div
              ref={scrollRef}
              className='flex flex-col gap-2 max-h-40 overflow-y-auto mb-2 pr-1'
            >
              <AnimatePresence>
                {comments.length === 0 ? (
                  <motion.div
            ...
            >
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && !e.shiftKey) handleCommentSend(e)
                }}
                maxLength={200}
                disabled={!user}
                autoComplete='off'
