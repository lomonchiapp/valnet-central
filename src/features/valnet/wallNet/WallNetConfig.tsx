...
const WallNetConfig = () => {
  ...
  const [moderatorRoles, setModeratorRoles] = useState<RoleUsuario[]>([
    RoleUsuario.ADMIN,
      ...
      <Tabs value={tab} onValueChange={setTab} className='w-full'>
        ...
        <TabsContent value='categories'>
          ...
          >
            ...
            <Button type='submit' disabled={loading} className='ml-0 md:ml-2'>
              Agregar
            </Button>
          </form>
            ...
            {categories.map((cat, idx) => (
              ...
              >
                {editId === cat.id ? (
                  <>
                    ...
                    >
                      <SelectTrigger className='w-[100px]'>
                        <SelectValue>
                          {editVisibility === 'public' ? 'Pública' : 'Privada'}
                        </SelectValue>
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value='public'>Pública</SelectItem>
                  ...
                  <>
                    <Badge
                      style={{ backgroundColor: cat.color || '#1976d2' }}
                    ...
                    <Badge
                      variant={
                        cat.visibility === 'private' ? 'secondary' : 'outline'
                      }
                    >
          ...
          <div className='text-xs text-muted-foreground mt-2'>
            Arrastra para reordenar categorías (solo local, no persistente aún).
          </div>
        </TabsContent>
        <TabsContent value='priorities'>
          <h3 className='font-semibold mb-2'>Gestión de prioridades</h3>
          <div className='text-muted-foreground'>
            (Próximamente: agregar, editar, eliminar y reordenar prioridades)
          </div>
        </TabsContent>
        <TabsContent value='permissions'>
          <h3 className='font-semibold mb-4'>Permisos y visibilidad</h3>
          <div className='mb-4 text-muted-foreground text-sm'>
            Configura qué roles pueden publicar, comentar y ver publicaciones en
            WallNet.
          </div>
          ...
          <div className='mt-4 text-xs text-muted-foreground'>
            Estos permisos aplican a todo WallNet. Para restricciones por
            categoría, usa la visibilidad de cada categoría.
          </div>
        </TabsContent>
          ...
          <div className='mb-6'>
            <div className='flex items-center gap-2 mb-2'>
              ...
              <label htmlFor='manual-approval' className='text-sm'>
                Requiere aprobación manual de publicaciones
              </label>
            </div>
            <div className='text-xs text-muted-foreground ml-7'>
              Si está activado, un moderador debe aprobar cada publicación antes
              de que sea visible.
            </div>
          </div>
          <div>
            ...
            <div className='flex flex-wrap gap-2'>
              {Object.values(RoleUsuario).map((role) => (
                <Button
                  key={role}
                  variant={
                    moderatorRoles.includes(role) ? 'default' : 'outline'
                  }
                  size='sm'
            ...
            <div className='text-xs text-muted-foreground mt-2'>
              Los moderadores pueden aprobar publicaciones y gestionar palabras
              prohibidas.
            </div>
          </div>
        </TabsContent>
          ...
          <div className='space-y-4'>
            <div className='flex items-center gap-4'>
              <Switch
                ...
                onCheckedChange={setNotifyNewPost}
                id='notify-new-post'
              />
              <label htmlFor='notify-new-post' className='text-sm'>
                Notificar nuevas publicaciones
              </label>
            </div>
            <div className='text-xs text-muted-foreground ml-10 mb-2'>
              Todos los usuarios conectados recibirán una notificación cuando se
              publique algo nuevo.
            </div>
            <div className='flex items-center gap-4'>
              <Switch
                checked={notifyMention}
                onCheckedChange={setNotifyMention}
                id='notify-mention'
              />
              <label htmlFor='notify-mention' className='text-sm'>
                Notificar menciones (@usuario)
              </label>
            </div>
            <div className='text-xs text-muted-foreground ml-10 mb-2'>
            ...
            <div className='flex items-center gap-4'>
              <Switch
                ...
                onCheckedChange={setNotifyComment}
                id='notify-comment'
              />
            ...
            <div className='text-xs text-muted-foreground ml-10'>
              El autor de la publicación recibirá una notificación cuando
              alguien comente.
            </div>
          </div>
        </TabsContent>
        <TabsContent value='advanced'>
          ...
          <div className='text-muted-foreground'>
            (Próximamente: comentarios, adjuntos, modo solo lectura, etc.)
          </div>
        </TabsContent>
      </Tabs>
    </div>
