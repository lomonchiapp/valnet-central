...
export const useListarFacturas = () => {
  ...
  const listarFacturas = useCallback(async (params: ListarFacturasParams = {}) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (isSuccess && response.facturas && Array.isArray(response.facturas)) {
        ...
        if (isSuccess) {
          ...
          const errorMsg = response.error || response.message || 'No se encontraron facturas API';
