...
import { PreRegistroMikrowisp, PreRegistro } from '@/types/interfaces/ventas/preRegistro';
...
interface UsePreRegistroReturn {
  loading: boolean;
  error: Error | null;
  crearPreRegistro: (preRegistro: Omit<PreRegistro, 'id' | 'updatedAt' | 'createdAt'>) => Promise<{ mikrowispSuccess: boolean; firestoreId?: string }>;
...
export const usePreRegistro = (): UsePreRegistroReturn => {
  ...
  const crearPreRegistro = async (preRegistroData: Omit<PreRegistro, 'id' | 'updatedAt' | 'createdAt'>) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      setError(err instanceof Error ? err : new Error('Error desconocido al crear pre-registro'));
  ...
  const misPreRegistros = async (): Promise<PreRegistro[]> => {
    ...
    try {
      ...
      setError(err instanceof Error ? err : new Error('Error desconocido al obtener pre-registros'));
      return [];
    } finally {
      setLoading(false);
    }
